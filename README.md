...
Item-list definitions can also include `draw_circle 1` to draw a circular volume around an item's pivot. A radius line runs from the pivot to the circle's front edge, following the item's MIS heading, so the pivot and orientation remain visible. The item's `wpdistance` value controls the radius in meters; `draw_circle 0` disables the visualization. Newly inserted KOTH center items default to a `wpdistance` of 20 meters.
